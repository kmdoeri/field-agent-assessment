...
public class AgencyAgentJdbcTemplateRepository implements AgencyAgentRepository {
    ...
    public List<AgencyAgent> findAll() {
        final String sql = "select aa.agency_id, aa.identifier, aa.activation_date, aa.is_active, " +
                ...
                " inner join security_clearance sc on sc.security_clearance_id = aa.security_clearance_id" +
                " inner join agent a on a.agent_id = aa.agent_id;";
        return jdbcTemplate.query(sql, new AgencyAgentMapper());
    }

    //No find by id or find all. No Get methods
    @Override
    public boolean add(AgencyAgent agencyAgent) { //add all of the columns
        //different than the add methods in agency and agent: no keyholder, returns boolean
    ...
    public boolean update(AgencyAgent agencyAgent) {
        ...
        final String sql = "update agency_agent set " //update anything other than agency and agent ids. Update is based off of those ids (primary key of table)
                + "identifier = ?, "
    ...
    public boolean deleteByKey(int agencyId, int agentId) {
        ...
        final String sql = "delete from agency_agent "
                + "where agency_id = ? and agent_id = ?;"; //delete based on agent and agency ids
